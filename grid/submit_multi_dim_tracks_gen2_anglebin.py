...
parser.add_argument('-c', dest='config', default="", help="a config file")
...
def run_grid(inputfiles):
    ...
    os.system('cp ../grid/bin/grid_executable_multi_dim_tracks_gen2_anglebin.sh %s' %MasterJobDir)
    ...
    cp_setup = "cp " + WIREMOD_WORKING_DIR + "/setup_grid_gen2.sh " + MasterJobDir
    cp_BashColorSets = "cp -r " + CALIB_WORKING_DIR + "/bin " + MasterJobDir
    cp_script = "cp " + WIREMOD_WORKING_DIR + "/macros/multi_dim_tracks_gen2_anglebin_grid.C " + MasterJobDir
    cp_config = "cp " + args.config + " " + MasterJobDir
...
"file://$(pwd)/grid_executable_multi_dim_tracks_gen2_anglebin.sh" \\
"%s" \\
"%s" \\
"%s"'''%(ngrid,OutputDir,args.output, config_name)
