...
void multi_dim_tracks_gen2_anglebin_grid(TString list_file, TString out_suffix, TString config
    ...
    Int_t kPlaneIndex = env.GetValue("PlaneIndex", -1);
    Float_t kThetaXWMin = env.GetValue("ThetaXWMin", -90.0);
    Float_t kThetaXWMax = env.GetValue("ThetaXWMax", 90.0);
    Float_t kThetaYZMin = env.GetValue("ThetaYZMin", -90.0);
    Float_t kThetaYZMax = env.GetValue("ThetaYZMax", 90.0);
    bool kUseAbsAngles = env.GetValue("UseAbsAngles", false);
    ...
    std::cout << "PlaneIndex: " << kPlaneIndex << " (global index ip + " << kNplanes << " * tpc)" << std::endl;
    std::cout << "UseAbsAngles: " << kUseAbsAngles << std::endl;
    std::cout << "ThetaXW selection: [" << kThetaXWMin << ", " << kThetaXWMax << ") degrees" << std::endl;
    std::cout << "ThetaYZ selection: [" << kThetaYZMin << ", " << kThetaYZMax << ") degrees" << std::endl;
	...
	float trk_thyz = -180.;
	  ...
	  get_dir(trk_thxz, trk_thyz, my.tpc[ip][i], ip, *my.trk_dirx, *my.trk_diry, *my.trk_dirz);
          ...
          if ((kPlaneIndex >= 0) && (hit_idx != static_cast<unsigned>(kPlaneIndex))) continue;

          float select_thxz = kUseAbsAngles ? std::abs(trk_thxz) : trk_thxz;
          float select_thyz = kUseAbsAngles ? std::abs(trk_thyz) : trk_thyz;
          bool pass_thxz = (select_thxz >= kThetaXWMin) &&
            ((select_thxz < kThetaXWMax) || (kThetaXWMax >= 90.0 && select_thxz <= kThetaXWMax));
          bool pass_thyz = (select_thyz >= kThetaYZMin) &&
            ((select_thyz < kThetaYZMax) || (kThetaYZMax >= 90.0 && select_thyz <= kThetaYZMax));
          if (!pass_thxz) continue;
          if (!pass_thyz) continue;
    ...
    out_rootfile -> cd();
    for (unsigned i = 0; i < kNplanes * kNTPCs; i++) {
        if ((kPlaneIndex >= 0) && (i != static_cast<unsigned>(kPlaneIndex))) continue;
